...
OCV0 = model.OCV0(:); % force to be col vector... 
OCVrel = model.OCVrel(:); % force to be col vector... 
if isscalar(temp), 
...
% for voltages less than 0% soc
% extrapolate off low end of table (for SOC(1) < 0...)
if ~isempty(I1),
  dv = (OCV0(2)+tempcol.*OCVrel(2)) - (OCV0(1)+tempcol.*OCVrel(1));
  ocv(I1)= (soccol(I1)-SOC(1)).*dv(I1)/diffSOC + OCV0(1)+tempcol(I1).*OCVrel(1);
end

% for voltages greater than 100% soc
% extrapolate off high end of table (for SOC(end) > 1..)
if ~isempty(I2),
...
I4=(soccol(I3)-SOC(1))/diffSOC; % for SOC(1) < 0
